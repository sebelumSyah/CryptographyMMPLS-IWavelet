function [X kode]=MultiAnalysisTypeMaxMinLS(A,level)
n=size(level,2);
kode=[];
X=A;
for k=1:n
    [X l]=MultiLevelAnalysisTypeMaxMinLS(X,level(k));
    kode=[l kode];
end
%==========================================================================
    function [c1 l1]=MultiLevelAnalysisTypeMaxMinLS(A2,level_value)
    n=size(A2,2);
    c1=[];l1=[];
    for t=1:level_value
        temp=A2;
        l1=[size(temp,2) l1];
        [temp le]=AnalysisMaxMinLS(A2);
        ...
        function [c l]=AnalysisMaxMinLS(A1)
        n1=size(A1,2);
